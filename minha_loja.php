...
$stmt->bind_param("i", $usuario_id); // Substitua pelo ID do usuário atual
$stmt->execute();
...
<body>
    ...
    <main>
     ...
     <?php
        ...
        // Consulta para buscar produtos do usuário logado
        $sql = "SELECT * FROM produtos WHERE usuario_id = ?";
        $stmt = $conn->prepare($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            // Percorra os produtos e divida-os em grupos de 5
            while ($row = $result->fetch_assoc()) {
                ...
                <div class="swiper-slide" id="swiper-card">
                    <div class="container-content-card">
                        ...
                        <h1 class="title-preço">R$ <?= number_format($row['preco'], 2, ',', '.') ?> <span class="span-porcent"> 47% off</span></h1>
                          <div class="link-adicionar-carrinho">
                              <a href="editar_produto.php?id=<?= htmlspecialchars($produto_id) ?>">Editar</a>
                          </div>
                        </div>
                    </div>
                </div>
            ...
            ?>
            <div class="swiper-card cardSwiper" style="padding: 0 1rem;">
                <div class="swiper-wrapper">
                    <div class="swiper-slide">Nenhum produto encontrado.</div>
                </div>
                <!-- <div class="swiper-pagination-card"></div> Adicionar paginação -->
            </div> <!-- Fechar `swiper-card` -->
            <?php
